...
def test_qmamba_one_token_prompt_chunk_waits_for_prompt_end(monkeypatch):
    monkeypatch.setenv("NS_GDN_QBITS", "4")
    builder = _create_gdn_builder(mamba_cache_mode="none", async_scheduling=False)
    batch = BatchSpec(seq_lens=[81, 49, 80], query_lens=[1, 1, 1])
    common = create_common_attn_metadata(batch, BLOCK_SIZE, DEVICE).replace(
    ...
    meta = builder.build(
        common_prefix_len=0,
        common_attn_metadata=common,
        qmamba_prefill_len_cpu=torch.tensor([80, 80, 80]),
        qmamba_num_computed_tokens_cpu=torch.tensor([80, 48, 79]),
    )
    assert meta.qmamba_quantize_d.tolist() == [True, False, True]


def test_qmamba_mixed_batch_keeps_decode_and_prefill_masks_aligned(monkeypatch):
    monkeypatch.setenv("NS_GDN_QBITS", "8")
    builder = _create_gdn_builder(mamba_cache_mode="none", async_scheduling=False)
    batch = BatchSpec(seq_lens=[81, 64, 80], query_lens=[1, 16, 16])
    common = create_common_attn_metadata(batch, BLOCK_SIZE, DEVICE)
    meta = builder.build(
        common_prefix_len=0,
        common_attn_metadata=common,
        qmamba_prefill_len_cpu=torch.tensor([80, 80, 80]),
        qmamba_num_computed_tokens_cpu=torch.tensor([80, 48, 64]),
    )
    assert meta.qmamba_quantize_d.tolist() == [True]
    assert meta.qmamba_quantize_p.tolist() == [False, True]


def test_qmamba_capture_does_not_quantize_dummy_rows(monkeypatch):
    monkeypatch.setenv("NS_GDN_QBITS", "8")
    builder = _create_gdn_builder(
        full_cuda_graph=True, mamba_cache_mode="none", async_scheduling=False
    )
    batch = BatchSpec(seq_lens=[1, 1], query_lens=[1, 1])
    common = create_common_attn_metadata(batch, BLOCK_SIZE, DEVICE)
    meta = builder.build_for_cudagraph_capture(common)
    assert meta.qmamba_quantize_d.tolist() == [False, False]
