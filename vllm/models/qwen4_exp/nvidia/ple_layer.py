...
def _get_ple_embedding_quant_method(
    ...
    # An explicitly declared PLE dtype wins over the model-wide quantization:
    # RadixArk/Qwen3.8-Flash-Next-NVFP4 is ModelOpt NVFP4 everywhere except the
    # n-gram tables, which it ships as FP8 shards plus one global scale. Testing
    # quant_config first would hand those shards to the NVFP4 method.
...
class Qwen4ExpNGramEmbedding(PleOffloadLayer):
    ...
    def compute_ngram_ids(
        ...
        if is_offload_process():
            ...
            # Upstream narrows the pack workspace to the live token count; a
            # request can never occupy more columns than that.
    ...
    def forward_impl(  # type: ignore[override]
        ...
        if is_offload_process():
            # The offload subprocess has no forward context and is never CUDA
            # graph captured, so the custom op indirection does not apply.
