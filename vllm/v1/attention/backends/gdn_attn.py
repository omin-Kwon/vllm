    ...
    def __init__(
        ...
        if self.qmamba_bits:
            ...
            if self.use_spec_decode:
                raise ValueError("Q-Mamba DSQ does not support speculative decoding")
            if vllm_config.cache_config.mamba_cache_mode != "none":
                raise ValueError("Q-Mamba DSQ requires --mamba-cache-mode none")
            if vllm_config.scheduler_config.async_scheduling:
                raise ValueError("Q-Mamba DSQ requires --no-async-scheduling")
        ...
        if self.qmamba_bits:
            ...
            if qmamba_for_cudagraph_capture:
                ...
                if (
                    ...
                    raise RuntimeError(
                        "Q-Mamba DSQ requires exact prefill lengths and "
                ...
                # One-token prompt chunks also use the decode kernel. They
                # must not be quantized until the logical prompt is complete.
                qmamba_quantize_d_cpu = (query_lens_cpu[:num_decodes] > 0) & (
                    qmamba_num_computed_tokens_cpu[:num_decodes]
                    + query_lens_cpu[:num_decodes]
                    >= qmamba_prefill_len_cpu[:num_decodes]
                )
                if num_prefills:
