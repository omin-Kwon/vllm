...
def create_cache(vllm_config, layer_idx, heads, head_dim, lower_bound):
    ...
    allowed = {"mode", "window", "checkpoint", "pivots", "flush_backend"}
    ...
    if mode == "replay":
        if any(k in options for k in ("checkpoint", "pivots", "flush_backend")):
    ...
    return SketchCache(
        ...
        flush_backend=options.get("flush_backend", "original"),
    )
