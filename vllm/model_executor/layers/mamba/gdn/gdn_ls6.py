...
호스트 몫이다 (scale/HANDOFF·bake_q38fn_ls6.py 머리말의 규약):
  ...
  ③ prefill 시딩  q̄/k̄ = 마지막 ≤W 토큰 평균(창끝 규약), 그 다음 Φ̄ 에필로그 커널
          ...
          꼬리 앵커 a_q/a_k 와 fz_u/z = S q̄_cold 를 만든다.
  ...
  ⑤ decode에서는 상태 의존 delta가 아니라 raw-WY write u_s와 projected factor f_s를
          저장한다. checkpoint 근사는 output에만 쓰인다.
...
def _ckpt():
    ...
    if _CKPT is None:
        ...
        if d.get("ls6_family") != "gdn" or d.get("ls6_embedded", True):
            raise ValueError(
                f"NS_GDN_LS6={_PATH}: gdn 계열·명시 기저"
                "(ls6_embedded=False) ckpt 가 아니다"
            )
        mt, nf, K = d["m_table"], d["nf_table"], int(d["K"])
        if not bool(((mt > 0) ^ (nf == K)).all()):
            raise ValueError(
                "ckpt 불변식 (m>0) xor (nf==K) 위반 — bake_ls6_dense 를 안 거쳤다"
            )
        idx = {}
...
class _Shared:
    """층별 슬롯 배정표. 블록 번호 공간이 달라 층마다 하나씩 둔다."""
    ...
    def init(self, NX, NS, dev):
        ...
        self.never = torch.full(
            (1,), 1 << 62, dtype=torch.int64, device=dev
        )  # 그래프 안에서 host 스칼라 복사 금지
        self.NS = NS


_LEADER: list[str | None] = [None]  # 로그·진단 카운터용 첫 활성 층
...
def configure_layer(layer, prefix: str, vllm_config=None):
    ...
    if vllm_config is not None:
        ...
        if vllm_config.cache_config.mamba_ssm_cache_dtype != "float32":
            raise RuntimeError(
                "NS_GDN_LS6: state 는 fp32 규약 (--mamba-ssm-cache-dtype float32)"
            )
    ...
    full_coordinates = os.environ.get("NS_GDN_FULL_COORDINATES", "0") == "1"
    if full_coordinates and (K, V, W, r, rep) != (128, 128, 16, 128, 3):
        raise ValueError("full-coordinate GDN requires K=V=r=128, W=16, rep=3")
    HK, HV = layer.num_k_heads, layer.num_v_heads
    if (layer.head_k_dim, layer.head_v_dim, HV // HK) != (K, V, rep):
        raise ValueError(
            f"ckpt 기하 (K,V,rep)=({K},{V},{rep}) != 층 "
            f"({layer.head_k_dim},{layer.head_v_dim},{HV // HK})"
        )
    if (
        vllm_config is not None
        and int(vllm_config.cache_config.replayssm_buffer_len) != W
    ):
        raise ValueError(
            f"ckpt W={W} != replayssm_buffer_len="
            f"{vllm_config.cache_config.replayssm_buffer_len}"
        )
    ...
    if (
        G > 64
        and os.environ.get("NS_GDN_STEP_IMPL", "cuda") == "cuda"
    ...
    # 모델 구성은 default device=cuda 문맥에서 돈다 — ckpt
    # 텐서(CPU)와 섞이지 않게 CPU 를 박는다.
    ...
    if bool(bad.any()):
        raise ValueError(
            f"층 {li}: Φ̄ 경로는 nf ∈ {{0(래치), K(dense)}} 만 — "
            f"nf={nf.tolist()} m={mt.tolist()}"
        )
    layer._ls6 = dict(
        li=li,
        K=K,
        ...
        mh=mt.to(torch.int32).to(dev).contiguous(),
        nf=nf.to(torch.int32).to(dev).contiguous(),
        cold=(torch.arange(K, device="cpu")[None, :] >= nf[:, None])
        .float()
        .to(dev),  # (HV,K)
    ...
    logger.info(
        "[ls6] %s: ckpt 층 %d  래치 head %d/%d  m max %d  "
        "G %d  r %d  W %d  Ω∩R[:r] %.2f..%.2f%s",
        prefix,
        li,
        int((mt > 0).sum()),
        HV,
    ...
    if full_coordinates:
        logger.info(
            "[ls6-full] %s: full-coordinate CUDA step/flush; anchors disabled", prefix
        )


# ─────────────────────────── ① 회전 ───────────────────────────
def rotate_mixed(layer, mixed_qkv):
    """conv 뒤 q,k 만 head 별로 제자리 회전한다: q' = R q."""
...
def _bufs(layer, ssm_state):
    ...
    if sh.map is not None and sh.map.shape[0] != NX:
        if layer._ls6_leader:
            logger.info(
                "[ls6] NX %d → %d: 슬롯 배정표·버퍼 재할당", sh.map.shape[0], NX
            )
        sh.map = None
        b = None
    ...
    if L6.get("full_coordinates", False):
        from vllm.third_party.flash_linear_attention.ops.gdn_full_runtime import (
            FullCoordinateRuntime,
        )

        layer._ls6_full_runtime = FullCoordinateRuntime(
            max(1, L6["max_num_seqs"]),
            HK,
            HV,
            G,
            dev,
        ...
        b.update(
            dict(
                aq=torch.zeros(NS, HV, G, **f32),
                ak=torch.zeros(NS, HV, G, **f32),
                fz_u=torch.zeros(NS, HV, V, **f32),
                fz_z=torch.zeros(NS, HV, V, **f32),
                qbar=torch.zeros(NS, HK, K, **f32),
                kbar=torch.zeros(NS, HK, K, **f32),
                qacc=torch.zeros(NS, HK, K, **f32),
                kacc=torch.zeros(NS, HK, K, **f32),
    ...
    if layer._ls6_leader:
        ...
        logger.info(
            "[ls6] 슬롯 버퍼 층당 %.0f MB (Ū %.0f MB, G=%d)",
            tot / 2**20,
            b["ubar"].numel() * 4 / 2**20,
            G,
        )
...
def _assign_slots(sh, pf_idx, has_init):
    """새 시퀀스에 compact 행을 준다. 기존 행을 재사용하거나 LRU 행을 쓴다.
    살아 있는 요청은 스텝마다 보이므로 희생자는
    ...
    for x, h in zip(xs, hi):
        ...
        if h:
            raise RuntimeError(
                f"[ls6] 블록 {x} 의 prefill 이어붙임인데 compact 행이 없다 "
                "— 슬롯이 뺏겼다(NS 부족)"
            )
        v = int(torch.argmin(sh.seen))
        if int(sh.seen[v]) >= step - 1 and int(sh.owner[v]) >= 0:
            raise RuntimeError(
                f"[ls6] compact 슬롯 부족: 희생자 행 {v} 가 "
                f"스텝 {int(sh.seen[v])} (지금 {step}) 에 보였다. "
...
def seed_prefill(layer, ssm_state, pf_idx, has_init, mixed_pf, cu_seqlens, scale):
    """prefill 뒤 정확한 state에서 시작한다. mixed_pf는 회전된
    ...
    c = sh.map[x].long()  # (n,)
    if L6.get("full_coordinates", False):
        layer._ls6_full_runtime.refresh(
            ssm_state,
            pf_idx,
            sh.map,
            L6["mh"],
    ...
    st, en = cu[:-1], cu[1:]
    pos = (
        en[:, None] - W + torch.arange(W, device=x.device)[None, :]
    )  # (n,W) 창끝 W 토큰
    valid = pos >= st[:, None]
    ...
    gdn_ls6_epilogue(
        ssm_state,
        pf_idx.to(torch.int32).contiguous(),
        torch.tensor([n], dtype=torch.int32, device=x.device),
        sh.map,
        b["qbar"],
        b["kbar"],
        L6["mh"],
    ...
    cold = L6["cold"]
    b["fz_u"][c] = torch.einsum(
        "nhvk,nhk->nhv", S, qbar.repeat_interleave(rep, 1) * cold
    )
    b["fz_z"][c] = torch.einsum(
        "nhvk,nhk->nhv", S, kbar.repeat_interleave(rep, 1) * cold
    )
    b["fs"][c] = 0
    b["beta"][c] = 0
    del S
...
# ─────────────────────────── ④ decode 부기 ───────────────────────────
@torch.no_grad()
def decode_bookkeep(layer, ssm_state, mixed_dec, beta_logits, idx, write_pos, scale):
    """커널 앞 device 연산. 그래프 캡처 가능하며 패딩은 행 0을 쓴다."""
    if not getattr(layer, "_ls6_active", False):
        return None
    L6 = layer._ls6
    b = _bufs(layer, ssm_state)
    HK, K, W = L6["HK"], L6["K"], L6["W"]
    sh = layer._ls6_sh
...
def kernel_kwargs(layer, ssm_state):
    ...
    if L6.get("full_coordinates", False):
        return dict(
            ...
            ls6_fs=b["fs"],
            ls6_r=L6["r"],
            ls6_map=layer._ls6_sh.map,
            ls6_beta=b["beta"],
            ls6_full_workspace=layer._ls6_full_runtime,
        )
    return dict(
        ls6_ubar=b["ubar"],
        ls6_phi=b["phi"],
...
# ─────────────────────────── 진단: 층별 dense 대조 ───────────────────────────
# NS_GDN_LS6_CHECK=1 이면 decode 스텝마다 같은 입력을 **LS6
# 없이**(dense, 행 복사본) 한 번 더
...
@torch.no_grad()
def check_begin(
    layer, ssm_state, d_cache, k_cache, g_cache, mixed, a, b, idx, write_pos, scale
):
    ...
    ridx = torch.where(
        valid, torch.arange(1, B + 1, device=x.device), torch.zeros_like(x)
    ).to(idx.dtype)
    out = torch.empty(
        B,
        1,
    ...
    expected_u = (
        S[1:, :, :, :G].transpose(-1, -2)
        * (torch.arange(G, device=S.device)[None, :] < L6["mh"][:, None])[
            None, :, :, None
        ]
    )
    du = (b_["ubar"][c] - expected_u).flatten(1).abs().max(
        1
    ).values / expected_u.flatten(1).abs().max(1).values.clamp(min=1e-6)
    return dict(
        S=S,
        out=out,
        x=x,
...
def check_end(layer, chk, ssm_state, out_ls6):
    ...
    e = (
        (got - ref).abs().max(1).values / ref.abs().max(1).values.clamp(min=1e-6)
    ).tolist()
    # 출력은 모델 dtype(bf16) 으로 저장되므로 |Δy| 는 반올림
    # 경계에서 1 ulp 가 튄다 — 원소별 ulp 로 나눈
    # 최대값과 다른 원소 수를 같이 찍는다. 항등 구성이면 "≤1 ulp, 몇 개" 여야 한다.
    ulp = torch.ldexp(
        torch.ones_like(ref), torch.frexp(ref.clamp(min=1e-30).abs())[1] - _MANT
    )
    ...
    if bool(fl.any()):
        ...
        es = "  flush ΔS/|S| " + " ".join(
            f"{v:.1e}"
            for v in (
                (Sg - Sr).flatten(1).abs().max(1).values
                / Sr.flatten(1).abs().max(1).values.clamp(min=1e-6)
            ).tolist()
        )
    logger.info(
        "[ls6-check] step %d %s wp %s slot %s |Δy|/|y| %s  ulp %s  n≠ %s  ΔŪ %s%s",
        chk["step"],
        layer._ls6_name,
        chk["wp"].tolist(),
        chk["c"],
        " ".join(f"{v:.1e}" for v in e),
        " ".join(f"{v:.1f}" for v in eu),
        " ".join(str(int(v)) for v in nd),
        " ".join(f"{v:.1e}" for v in chk["du"]),
        es,
    )
