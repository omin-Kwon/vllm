...
"""Q-Mamba dynamic state quantization for Gated DeltaNet state caches."""
...
def bits_from_env() -> int:
    ...
    if granularity != "dsq_qm":
        raise ValueError(
            "Flash-Next Q-Mamba supports DSQ only: "
            f"NS_GDN_QGRAN must be 'dsq_qm', got {granularity!r}"
        )
    if os.environ.get("NS_GDN_QSR", "0") != "0":
        raise ValueError(
            "Flash-Next Q-Mamba DSQ uses deterministic round-to-nearest; "
            "NS_GDN_QSR is unsupported"
        )
    return bits
...
def quantize_slots_(
    ...
    """Fake-quantize selected ``(value, key)`` GDN states in place."""
    if bits not in _SUPPORTED_BITS:
        raise ValueError(f"unsupported Q-Mamba bit width: {bits}")
    if not state.is_cuda:
        raise RuntimeError("Q-Mamba GDN DSQ requires a CUDA state cache")
    ...
    if state.ndim != 4:
        raise RuntimeError(
            "Q-Mamba GDN DSQ expects state layout (slots, heads, value, key); "
